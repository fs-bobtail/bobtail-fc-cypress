class CardsPage {
  ...
  // Security TOGGLES
  securityToggle = () =>
    cy.get('.columns-template .toggle-slider input[type="checkbox"]');
  ...
  limitInputs = () =>
    '.modal-content-container input[data-cy="base-input-field"]';
  ...
  // Ellipse menu
  cardRowsEllipse = () =>
    ".cards-listing .columns-template:not(.card-table-header)";
  firstCardEllipsis = () =>
    '.cards-listing .columns-template:not(.card-table-header):first-child [data-cy="cardActions-menu"]';
  ...
  navigateToCardsPage() {
    cy.get(".fc-base-modal").should("not.exist");
    this.menuOption().should("be.visible").and("contain.text", "Cards").click();
    cy.url().should("eq", "https://card-app.bobtailtest.com/cards");
  }

  enableCardSecurity(index = 0) {
    this.cardToggleInput(index).click({ force: true });
    cy.contains("Card Unlock Security").should("be.visible");
    cy.contains("button", "Enable").should("be.visible").click();
    cy.contains("Card security Enabled for").should("be.visible");
  }

  disableCardSecurity(index = 0) {
    this.cardToggleInput(index).click({ force: true });
    cy.contains("Card Unlock Security").should("be.visible");
    cy.contains("button", "Disable").should("be.visible").click();
    cy.contains("Card security Disabled for").should("be.visible");
  }

  clickFirstNonFuelToggle() {
    cy.wait(500);
    this.cardToggleInput(1).click({ force: true });
  }
  ...
  enableNonFuelLimits() {
    ...
    cy.contains("Card settings saved successfully").should("be.visible");
  ...
  verifyFiltersVisible() {
    ...
    cy.get(this.filterActive()).should("be.visible");
    cy.get(this.filterInactive()).should("be.visible");
    cy.get(this.filterClosed()).should("be.visible");
  }

  waitForCardsToLoad() {
    cy.intercept("GET", "https://card-api.bobtailtest.com/cards").as(
      "getCards"
    );
  ...
  closeModal() {
    cy.get(this.modalCloseButton()).should("be.visible").click({ force: true });
  }
}
